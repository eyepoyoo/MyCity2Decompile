...
public class IRVExample : MonoBehaviour
{
	...
	private void Start()
	{
		...
		{
			internetReachabilityVerifier = (InternetReachabilityVerifier)Object.FindObjectOfType(typeof(InternetReachabilityVerifier));
			if (internetReachabilityVerifier == null)
			{
		...
		for (int i = 0; i < num; i++)
		{
			methodNames[i] = ((InternetReachabilityVerifier.CaptivePortalDetectionMethod)i).ToString();
		}
	}

	private void OnGUI()
	{
		if (logChosenDefaultByPlatformMethodPending && internetReachabilityVerifier.captivePortalDetectionMethod != InternetReachabilityVerifier.CaptivePortalDetectionMethod.DefaultByPlatform)
		{
