...
public class UICamera : MonoBehaviour
{
				...
				if (array.Length > 1)
				{
					...
					for (int num = 0; num < mHits.size; num++)
					{
						if (IsVisible(ref mHits.buffer[num]))
						{
							...
							lastWorldPosition = mHits[num].point;
							mHits.Clear();
							return true;
						}
					}
	...
	public void ProcessTouch(bool pressed, bool released)
	{
		...
		if (currentTouch.pressed != null)
		{
			...
			if (currentTouch.pressed == currentTouch.current && currentTouch.clickNotification != ClickNotification.None && !currentTouch.dragStarted && currentTouch.deltaTime > tooltipDelay)
			{
				currentTouch.clickNotification = ClickNotification.None;
				if (longPressTooltip)
				{
