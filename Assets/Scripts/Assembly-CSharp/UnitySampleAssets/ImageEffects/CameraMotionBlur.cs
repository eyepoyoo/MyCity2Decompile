	...
	public class CameraMotionBlur : PostEffectsBase
	{
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			if (filterType == MotionBlurFilter.CameraMotion)
			{
				...
				float num5 = 1f;
				num5 = Vector3.Angle(base.transform.up, prevFrameUp) / GetComponent<Camera>().fieldOfView * ((float)source.width * 0.75f);
				zero.x = rotationScale * num5;
				num5 = Vector3.Angle(base.transform.forward, prevFrameForward) / GetComponent<Camera>().fieldOfView * ((float)source.width * 0.75f);
