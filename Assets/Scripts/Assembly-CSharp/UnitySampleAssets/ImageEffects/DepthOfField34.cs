...
namespace UnitySampleAssets.ImageEffects
{
	...
	public class DepthOfField34 : PostEffectsBase
	{
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			float num2 = focalSize / (GetComponent<Camera>().farClipPlane - GetComponent<Camera>().nearClipPlane);
			if (simpleTweakMode)
			{
				...
				flag = flag && focalPoint > GetComponent<Camera>().nearClipPlane + Mathf.Epsilon;
			}
			else
			{
				if ((bool)objectFocus)
