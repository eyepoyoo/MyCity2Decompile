...
public class VCSBehaveTraverseLinkWaitOnThreat : VCSBehaveBase
{
	private enum STATE
	{
		VCS_TLD_UNSET = 0,
		...
		VCS_TLD_FINISHED = 3,
		VCS_TLD_WAITING_ON_THREAT = 4,
		NUM_VCS_TLD_STATES = 5
	}
	...
	public VCSBehaveTraverseLinkWaitOnThreat()
	{
	...
	protected override void OnUpdate()
	{
		...
		case STATE.VCS_TLD_MOVING:
			UpdateMoving();
			break;
		case STATE.VCS_TLD_WAITING_ON_THREAT:
			UpdateWait();
			break;
	...
	private void UpdateMoving()
	{
		...
		else if (base._pOwnerBrain._pHasTargetValid)
		{
			SwitchToState(STATE.VCS_TLD_WAITING_ON_THREAT);
		}
		else if (!_hasMoved)
		{
	...
	{
		if (!base._pOwnerBrain._pHasTargetValid)
		{
			SwitchToState(STATE.VCS_TLD_MOVING);
		}
	}
